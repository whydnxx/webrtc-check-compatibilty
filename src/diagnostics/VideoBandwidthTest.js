...
export default class VideoBandwidthTest extends Test {
  constructor() {
    ...
    this.bweStats = new StatisticsAggregate(
      0.75 * this.maxVideoBitrateKbps * 1000
    );
  ...
  completed() {
    ...
    if (isWebkit) {
      ...
      if (this.videoStats[0] < 2 && this.videoStats[1] < 2) {
        ...
        this.addLog(
          "info",
          `Send bandwidth estimate max: ${stats.mbpsMax} mbps`
        );
        this.addLog(
          "info",
          `Send bandwidth ramp-up time: ${stats.rampUpTimeMs} ms`
        );
      }
    } else if (isFirefox) {
      if (parseInt(this.framerateMean, 10) > 0) {
        this.addLog(
          "info",
          `Frame rate mean: ${parseInt(this.framerateMean, 10)}`
        );
      } else {
        this.addLog(
          "error",
          "Frame rate mean is 0, cannot test bandwidth without a working camera."
        );
      }
      ...
      this.addLog(
        "info",
        `Send bitrate standard deviation: ${stats.mbpsStdDev} mbps`
      );
    }
